            ...
            // STATE 2: Якщо раб не повний - збирати ресурс
                ...
                if (distToResource < 2.0f) {
                    ...
                    if (!unit.is_moving || !(unit.target_position == resourcePos)) {
                        unit.moveTo(resourcePos);
                        LOG_BUILDING("[MOVE_TO_RESOURCE] Moving to resource at grid(%d,%d), distance=%.1f\n", 
...
void InitBuildings() {
    ...
    LOG_BUILDING("[BUILDINGS] Initialization complete: %d buildings spawned\n", (int)buildings.size());
}
...
void SpawnUnit(const Building& building, const std::string& unitType) {
    ...
    LOG_SPAWN("[SPAWN] Unit spawned at grid(%d, %d) screen(%d, %d)\n", 
            ...
            if (!foundEnemy) {
                ...
                if (!foundResource) {
                    ...
                    if (isPathBlockedByBuilding(units[0].getGridPosition(), gridPos)) {
                        LOG_CLICK("[MOVE] Cannot move to grid(%d,%d) - blocked by building\n", 
                        ...
                        for (int i = 0; i < units.size(); i++) {
                            if (units[i].selected && units[i].faction == playerFaction) {
                                ...
                                LOG_CLICK("[MOVE] Unit %d moving to grid(%d,%d)\n", i, gridPos.row, gridPos.col);
                            }
                        }
                    }
                }
